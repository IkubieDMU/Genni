...
import com.example.genni.viewmodels.AuthState
...
fun LoginScreen(nc: NavController, viewModel: AuthViewModel) {
    ...
    Box(modifier = Modifier.fillMaxSize().background(Brush.verticalGradient(listOf(emeraldGreen, deepPurple, deepPurple))), contentAlignment = Alignment.Center) {
        Column(modifier = Modifier.fillMaxWidth().padding(32.dp), horizontalAlignment = Alignment.CenterHorizontally) {
            ...
            // Username TextField
            UsernameTF(viewModel.username, {viewModel.onUsernameChange(it)}, "Username", Icons.Default.Person, "User Icon")

            Spacer(modifier = Modifier.height(10.dp))

            //Password TextField
            PasswordTF(viewModel.password, {viewModel.onPasswordChange(it)}, "Password", Icons.Default.Lock, "Password Icon")
